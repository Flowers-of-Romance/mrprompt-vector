...
結果3で律速は routing 精度だと分かった。では routing 精度を上げる手段を入れたとき、適合度はどこまで動くか。二系統で測る。一つは埋め込み検索器を取り替えて top-1 精度（R@1）の幅を出す。もう一つは facet 本体を読んで選ぶ LLM ルーターである。前者は手法ごとに生成・採点を測り直さず、R@1 から body_top1 の当たり/外し実測値（7.74／7.31）で適合度を換算する（投影）。後者は結論の分岐点になるため、選んだ facet を実際に Qwen3-8B に注入して生成・採点し、実測する。実測のアンカーは oracle（正解 facet を直接注入）＝7.88、all-facets＝7.43。
...
| bge-m3 colbert 近似 | 0.38 | 0.71 | 7.47 |
...
R@1 は 0.19–0.40 に収まり、bm25 を除けば 0.27–0.40 に集中する。中国語特化（0.40）、cross-encoder（0.27）、生成モデルの隠れ状態（0.27）を入れても 0.40 を超えない。投影適合度はいずれも all-facets（7.43）の ±0.05 に収まる。結果3の null は、埋め込み手法の選択に対して動かない。同一キャラ facet の意味的近接は、埋め込みの差し替えでは縮まらない。

ただしこの投影には前提がある。当たり/外しの平均（7.74／7.31）を body_top1 で代表させ、全手法で一定とみなしている。当たり群の母集団が手法ごとに違えば、この換算はずれる。次の LLM ルーターがその実例になる。
...
- 投影との対照。投影では router 7.56 ＞ two-stage 7.53 で、実測の順位（two-stage 7.79 ＞ router 7.62）と逆だった。投影が当たり群を body_top1 の 7.74 で代表させたのに対し、LLM ルーターの当たり群は 8.1–8.3 だった。当たり群の母集団が手法依存であることが、順位を変えるほど大きかった。LLM ルーターを投影で済ませず実測した理由がこれである。
...
限界：実測は LLM ルーター2条件のみで、埋め込み11手法は body_top1 の当たり/外しからの投影にとどまる。LLM 採点（GPT-4.1-mini）の判定器バイアスは全条件に共通で残る。
