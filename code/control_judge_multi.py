# -*- coding: utf-8 -*-
"""
control_judge_multi.py -- score every draw of the multi-sample deconfound run.

Same rubric / judge / temperature as control_judge (ADH_SYS, GPT-4.1-mini, temp 0), but one
score per (id, condition, sample). Resumable: skips draws already in the output. Records
without a "sample" field (the original 400) are scored as sample 0.

Reads  data/generations_control.jsonl + data/instances_faithful.jsonl
Writes data/scores_control_multi.jsonl (one row per draw: id, condition, sample, adh)
...
OUT = f"{BASE}/data/scores_control_multi.jsonl"
...
def main():
    ...
    if os.path.exists(OUT):
        for l in open(OUT):
            try:
                d = json.loads(l); done.add((str(d["id"]), d["condition"], int(d["sample"])))
            except Exception:
                pass
    todo = [g for g in gens if (str(g["id"]), g["condition"], int(g.get("sample", 0))) not in done]
    print(f"{len(gens)} draws, {len(todo)} to score")

    def score_one(g):
        it = insts[g["id"]]
        true_facet = facets_of(it["schema"])[it["cued_index"]]
        return (g["id"], g["condition"], int(g.get("sample", 0)),
                adherence(true_facet, it["stm"], g["response"]))

    out = open(OUT, "a")
    n = 0
    with ThreadPoolExecutor(max_workers=8) as ex:
        for gid, cond, samp, sc in ex.map(score_one, todo):
            out.write(json.dumps({"id": gid, "condition": cond, "sample": samp, "adh": sc},
                                 ensure_ascii=False) + "\n")
            out.flush(); n += 1
    out.close()
    print(f"scored {n} new draws -> {OUT}")
